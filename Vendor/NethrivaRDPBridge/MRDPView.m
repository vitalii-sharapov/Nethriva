...
static UINT mac_DisplayControlCaps(DispClientContext *disp, UINT32 maxNumMonitors,
...
- (int)rdpStart:(rdpContext *)rdp_context
{
	...
	mfc->client_height = self.bounds.size.height;
	mfc->client_width = self.bounds.size.width;
...
- (void)keyDown:(NSEvent *)event
{
	...
	if ((modifiers & NSEventModifierFlagCommand) &&
	    ...
	    [shortcut length] == 1)
	{
		const unichar character = [shortcut characterAtIndex:0];
		if (character == 'v')
		{
			[self pasteFromMacClipboard];
			return;
		}
		if (character == 'a' || character == 'c' || character == 'x')
		{
			DWORD shortcutScancode = RDP_SCANCODE_KEY_A;
			if (character == 'c')
				shortcutScancode = RDP_SCANCODE_KEY_C;
			else if (character == 'x')
				shortcutScancode = RDP_SCANCODE_KEY_X;

			releaseFlagStates(instance->context->input, kbdModFlags);
			kbdModFlags = 0;
			freerdp_input_send_keyboard_event(instance->context->input, KBD_FLAGS_DOWN,
			                                  RDP_SCANCODE_CODE(RDP_SCANCODE_LCONTROL));
			freerdp_input_send_keyboard_event(instance->context->input, KBD_FLAGS_DOWN,
			                                  RDP_SCANCODE_CODE(shortcutScancode));
			freerdp_input_send_keyboard_event(instance->context->input, KBD_FLAGS_RELEASE,
			                                  RDP_SCANCODE_CODE(shortcutScancode));
			freerdp_input_send_keyboard_event(instance->context->input, KBD_FLAGS_RELEASE,
			                                  RDP_SCANCODE_CODE(RDP_SCANCODE_LCONTROL));
			return;
...
- (void)keyUp:(NSEvent *)event
{
	...
	const NSEventModifierFlags modifiers =
	    [event modifierFlags] & NSEventModifierFlagDeviceIndependentFlagsMask;
	if ((modifiers & NSEventModifierFlagCommand) && [shortcut length] == 1)
	{
		const unichar character = [shortcut characterAtIndex:0];
		if (character == 'a' || character == 'c' || character == 'v' || character == 'x')
			return;
	}
...
{
	WINPR_UNUSED(timer);
	if (!mfc || !mfc->clipboard || !mfc->cliprdr || !pasteboard_rd)
		return;
	...
	NSString *formatString = [pasteboard_rd stringForType:NSPasteboardTypeString];
	ClipboardEmpty(mfc->clipboard);
	if (formatString)
	{
		NSMutableData *formatData =
		    [[[formatString dataUsingEncoding:NSUTF16LittleEndianStringEncoding] mutableCopy]
		        autorelease];
		const uint16_t terminator = 0;
		[formatData appendBytes:&terminator length:sizeof(terminator)];
		ClipboardSetData(mfc->clipboard, CF_UNICODETEXT, [formatData bytes],
		                 (UINT32)[formatData length]);
	}
...
- (void)pasteFromMacClipboard
{
	if (!is_connected || !instance || !instance->context)
		return;

	[self onPasteboardTimerFired:nil];
	releaseFlagStates(instance->context->input, kbdModFlags);
	kbdModFlags = 0;

	/* Let the clipboard channel accept a freshly announced Unicode format
	 * before issuing the Windows paste shortcut. */
	dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 100 * NSEC_PER_MSEC),
	               dispatch_get_main_queue(), ^{
		[self sendWindowsPasteShortcut];
	});
}
...
- (void)sendWindowsPasteShortcut
{
	if (!is_connected || !instance || !instance->context)
		return;
	rdpInput *input = instance->context->input;
	freerdp_input_send_keyboard_event(input, KBD_FLAGS_DOWN,
	                                  RDP_SCANCODE_CODE(RDP_SCANCODE_LCONTROL));
	freerdp_input_send_keyboard_event(input, KBD_FLAGS_DOWN,
	                                  RDP_SCANCODE_CODE(RDP_SCANCODE_KEY_V));
...
- (BOOL)pasteFilesAtPaths:(NSArray<NSString *> *)paths
{
	return [self pasteFilesAtPaths:paths activateX:-1 y:-1];
}

- (BOOL)pasteFilesAtPaths:(NSArray<NSString *> *)paths activateX:(double)x y:(double)y
{
	if (!is_connected || !mfc || !mfc->clipboard || !mfc->clipboardFile ||
	    !mfc->cliprdr || paths.count == 0)
		return NO;
	...
	NSData *uriData = [uriList dataUsingEncoding:NSUTF8StringEncoding];
	if (uriData.length == 0 || uriData.length > UINT32_MAX)
		return NO;

	const char *bytes = (const char *)uriData.bytes;
	const size_t length = uriData.length;
	if (!cliprdr_file_context_update_client_data(mfc->clipboardFile, bytes, length))
		return NO;

	ClipboardEmpty(mfc->clipboard);
	const UINT32 uriFormat = ClipboardRegisterFormat(mfc->clipboard, "text/uri-list");
	if (uriFormat == 0 || !ClipboardSetData(mfc->clipboard, uriFormat, bytes, (UINT32)length))
		return NO;

	/* Do not let the pasteboard polling timer replace this file offer with
	 * unrelated text before Windows requests the descriptors and contents. */
	pasteboard_changecount = (int)[[NSPasteboard generalPasteboard] changeCount];
	[self.window makeFirstResponder:self];
	if (x >= 0 && y >= 0)
	{
		const int remoteX = (int)MIN(MAX(x, 0), self.bounds.size.width - 1);
		const int remoteY = (int)MIN(MAX(y, 0), self.bounds.size.height - 1);
		mf_scale_mouse_event(context, PTR_FLAGS_MOVE, remoteX, remoteY);
		mf_press_mouse_button(context, 0, remoteX, remoteY, TRUE);
		mf_press_mouse_button(context, 0, remoteX, remoteY, FALSE);
	}

	mfc->pendingFilePaste = TRUE;
                                   ...
                                   UINT32 maxMonitorAreaFactorB)
{
	...
	mfContext *mfc = (mfContext *)disp->custom;
	mfc->displayControlReady = TRUE;
...
static void mac_OnChannelConnectedEventHandler(void *context, const ChannelConnectedEventArgs *e)
{
	...
	else if (strcmp(e->name, DISP_DVC_CHANNEL_NAME) == 0)
	{
		...
		mfc->disp->custom = mfc;
		mfc->disp->DisplayControlCaps = mac_DisplayControlCaps;
	}
...
BOOL mac_desktop_resize(rdpContext *context)
{
	...
	if (mfc->client_width <= 0)
		mfc->client_width = view.bounds.size.width;
	if (mfc->client_height <= 0)
		mfc->client_height = view.bounds.size.height;
	EventArgsInit(&e, "mfreerdp");
